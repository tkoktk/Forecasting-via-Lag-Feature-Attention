...
def log_returns(close):
    ...
    print(f"Log returns computed: {lr.notna().sum()} valid observations, {lr.isna().sum()} NaN")
    return lr
...
def forward_log_return(close, horizon):
    ...
    print(f"Forward log return (h={horizon}): {target.notna().sum()} valid targets, {target.isna().sum()} NaN (expected {horizon} at end)")
    return target
...
def parkinson_volatility(high, low, window):
    ...
    print(f"Parkinson vol (window={window}): {park_vol.notna().sum()} valid observations")
    return park_vol