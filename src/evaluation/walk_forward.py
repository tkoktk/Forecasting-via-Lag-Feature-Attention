...
def run_evaluation(configs, prices_df, tickers, date_start, date_end, horizon, step, verbose=True, return_predictions=False):
    results = []
    ...
    for config_name, config_params in configs.items():
        ...
        for ticker in tickers:
            ...
            y_true, y_pred = walk_forward(ticker_prices, config_params, horizon=horizon, step=step)
            ...
            if verbose:
                print(f"{ticker}: IC={metrics['ic']:.4f} (p={metrics['p_value']:.4f}), "
    ...
    if verbose:
        ...
        print(results_df.groupby("config")[["ic", "diracc", "mae", "rmse"]].mean().sort_values("ic", ascending=False))
